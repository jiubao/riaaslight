...
const defaultEventOptions = () => (supportPassive ? { capture: false, passive: true } : false);

export const on = (
  element: HTMLElement | Document | Window | FileReader | MediaSource | SourceBuffer,
  event: string,
...
export const hasClass = (elm: HTMLElement, className: string) =>
  elm.className && new RegExp('(^|\\s)' + className + '(\\s|$)').test(elm.className);
